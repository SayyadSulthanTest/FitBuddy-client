...
function ServerDay(props) {
  ...
  const isSelected =
    !outsideCurrentMonth &&
    highlightedDays.some((date) => date.startsWith(specificDateString));

  const isBeforeCurrentDate = specificDate.isBefore(dayjs(), "day");
  const isAfterStartDate =
    specificDate.isSame(startDate, "day") ||
    specificDate.isAfter(startDate, "day");
...
export default function DateCalendarServerRequest({ progress, challenge }) {
  ...
  const highlightedDays = useMemo(() => {
    const filteredData = dataFromServer
      ...
      .filter((item) =>
        dayjs(item.date).isBefore(currentDateLocalISOString, "day")
      )
